...
class Tile:
    ...
    def has_enough_tissue(
        ...
        if not self._has_tissue_more_than_percent(tissue_percent):
            return False
        if self._is_almost_black:
            # self._image.save('F:/test\delete/'+str(self.coords)+'.png')
            return False
    ...
    @lazyproperty
    def _is_almost_black(self) -> bool:
        """Check if the image is almost black.
        ...
        bool
            True if the image is almost white, False otherwise
        """
        image_black = np.array(self._image)
        return (np.mean(image_black) < 50 or np.std(image_black) < 10
        )
